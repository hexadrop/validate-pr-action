...
1. Open an issue using the [Bug Report](https://github.com/hexadrop/validate-pr-action/issues/new?template=bug_report.yml) or [Feature Request](https://github.com/hexadrop/validate-pr-action/issues/new?template=feature_request.yml) template.
...
```
feat(scope): validate changeset requirements per PR type
fix(scope): accept exempt Renovate pull requests
```
...
- [ ] Changeset added for any behavior change (`bun changeset add`)
- [ ] `dist/index.js` regenerated with `bun run build` and committed
- [ ] Code self-reviewed

## Changesets

If the PR changes the action's behavior, add a changeset:
